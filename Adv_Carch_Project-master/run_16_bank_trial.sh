...
run_cache_trial() {
  ...
  (
    ...
    sed -i 's/num-banks=.*/num-banks=16/g' config.ini

    echo "  banks-to-attack=1 (single bank out of 16, others idle)"
    sed -i 's/banks-to-attack=.*/banks-to-attack=1/g' config.ini
    python3 main.py &
    sleep 1

    echo "  banks-to-attack=16 (all 16 banks simultaneously)"
    sed -i 's/banks-to-attack=.*/banks-to-attack=16/g' config.ini
    python3 main.py &
    sleep 1

    wait
    echo "  $cache 16-bank runs complete."
...
echo "All 16-bank runs and plots complete."
